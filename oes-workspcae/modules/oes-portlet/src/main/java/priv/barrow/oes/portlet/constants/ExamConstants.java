package priv.barrow.oes.portlet.constants;

public class ExamConstants {
    public static final String EXAM = "exam";
    public static final String EXAMS = "exams";
    public static final String EXAM_ID = "examId";
    public static final String EXAM_NAME_FIELD_NAME = "ExamName";
    public static final String QUESTION_COUNT_FIELD_NAME = "QuestionCount";
    public static final String QUESTION_COUNT = "questionCount";
    public static final long INEXISTENT_EXAM_ID = -1L;
    public static final String EXAM_STRUCTURE_NAME = "Exam";
    public static final String CLASS_NAME_ID = "classNameId";
    public static final String CLASS_PK = "classPK";
    public static final String FIELDS = "fields";
    public static final String CREATE_EXAM = "createExam";
    public static final String DDM_STRUCTURE_ID = "DDMStructureId";
    public static final String RECORD_SET_ID = "recordSetId";
    public static final String USER_ID = "userId";
    public static final String RESULT_MAP = "resultMap";
    public static final String REVIEW_EXAM_URL_WITH_EXAM_STUDENT_ID = "/review-exam?examId=%d&studentId=%d";

}
